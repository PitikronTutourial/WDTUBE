<?php 
    ...
    if(isset($_POST['update_profile'])){
        ...
        mysqli_query($conn, "UPDATE `user` SET username = '$update_name', email = '$update_email' WHERE user_id = '$user_id'") or die('query failed');
     
        $update_image = $_FILES['update_image']['name'];
        $update_image_size = $_FILES['update_image']['size'];
        $update_image_tmp_name = $_FILES['update_image']['tmp_name'];
        $update_image_folder = 'uploaded_profile/'.$update_image;
     
        if(!empty($update_image)){
           if($update_image_size > 5000000){
              $message[] = 'image is too large';
           }else{
              $image_update_query = mysqli_query($conn, "UPDATE `user` SET image = '$update_image' WHERE user_id = '$user_id'") or die('query failed');
    ...
    <div class="content">
        ...
        <div class="action-bar">
            <div class="tabcontent">
                <a href="\WDTube\php\customization1.php"><div class="title active"><span>Your info</span></div></a>
                <a href="\WDTube\php\customization2.php"><div class="pass"><span>Change Password</span></div></a>
            </div>
            <div class="bttab">
              <form action="\WDTube\php\user-profile.php" method="POST">
              <input type="text" name="userid" class="hidden" value="<?php echo $user_id?>">
                <input type="submit" class="channel" value="VIEW CHANNEL">
              </form>
            </div>
        </div>
        <div class="info">
