<?php
    ...
    $pl_id = $_GET['playlist_id'] ;
    $video_code = $_GET['video_code'] ;

    $pl_video = mysqli_query($conn, "SELECT * FROM playlist INNER JOIN video ON playlist.video_id = video.video_id INNER JOIN user ON video.user_id = user.user_id WHERE $pl_id = playlist_id") ;
    $pl_video_result = mysqli_fetch_assoc($pl_video) ;
    ...
    $keep_history = mysqli_query($conn,"SELECT * FROM history INNER JOIN video ON history.video_id = video.video_id INNER JOIN user ON history.user_id = user.user_id WHERE video.video_code = '$video_code' ") ;
...
<head>
  ...
  <title>WDTube</title>
  <link rel="stylesheet" href="\WDTube\css\playlist_view.css">
        ...
        <div class="row">
            <div class="playvideo">
                ...
                <div class="plublisher">
                    ...
                    ?>
                    <div>
                        <form action="/WDTube/php/channel.php" method="POST">
                            <input type="text" name="userid" class="hidden" value="<?php echo $recordch?>">
                        <?php
            ...
            <div class="rightsidebar">
                <?php 
                    $pl_detail = mysqli_query($conn, "SELECT * FROM create_playlist INNER JOIN user ON create_playlist.user_id = user.user_id WHERE $pl_id = playlist_id");
                    $pl_result = mysqli_fetch_assoc($pl_detail);
                ?>
                <!-- (A) TOGGLE BUTTON -->
                <label for="check" class="togButton"><?php echo $pl_result['playlist_name'];?><br>
                    <p style="font-size: 13px; font-weight: normal; margin-top: 5px;">Created by : <?php echo $pl_result['username'];?></p>
                </label>
                <!-- (B) CONTENT -->
                <input type="checkbox" class="togCheck" id="check"/>
                <div class="togContent">
                    <div class="video-list-container">
                        <a href="\WDTube\php\playlist_view.php?playlist_id=<?php echo $pl_id ?>&video_code=<?php echo $pl_video_result['video_code']?>">
                        <div class="list active">  
                            <div class="video">
                                <div class="video_column">
                                    <div class="video_thumbnail">
                                        <?php
                                        echo '<img src="uploaded_thumbnail/'.$pl_video_result['video_pic'].'"></a>';
                                        ?>
                                        <div class="video_name_css">
                                            <?php
                                            echo '<a style="text-decoration: none;" href="\WDTube\php\playlist_view.php?playlist_id='.$pl_id.'&video_code='.$pl_video_result['video_code'].'"><h2>'.$pl_video_result['video_name'].'</h2></a>';
                                            ?>
                                            <div class="video_des_css">
                                                <?php
                                                echo '<a style="text-decoration: none;" href="\WDTube\php\playlist_view.php?playlist_id='.$pl_id.'&video_code='.$pl_video_result['video_code'].'"><h3>'.$pl_video_result['username'].'</h3></a>';
                                                ?>
                                            </div>
                                        </div>
                                    </div>  
                        ...
                        <?php 
                        while($pl_video_result = mysqli_fetch_assoc($pl_video))
                        {
                        ?>
                        <div class="list active">
                            <a href="\WDTube\php\playlist_view.php?playlist_id=<?php echo $pl_id ?>&video_code=<?php echo $pl_video_result['video_code']?>">
    ...
    <div class="bg-modal">
        <div class="modal-contents">
            ...
            <form action="update.php" class='inpop scroll' method="POST">
                <?php
                    $cresult = mysqli_query($conn, 'SELECT * FROM create_playlist ORDER BY playlist_id');
